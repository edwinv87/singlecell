...
class SingleCell:
    ...
    def getLogCounts(self):
        ...
        # Check if gene filtering has been done. Return filtered genes
        if (self.checkGeneDataColumn("gene_filter") == True):
            self.excl_genes = self.excl_genes & self.genedata["gene_filter"].values
    ...
    def addGeneDataColumn(self, col_data, col_name):
        ...
        if (self.checkGeneDataColumn("gene_filter")):
            gen_filt = self.genedata["gene_filter"].values
            data = np.zeros(self.dim[0])
            data[gen_filt] = col_data
    ...
    def addCellDataColumn(self, col_data, col_name):
        ...
        data = np.zeros(self.dim[1], col_data.dtype)
